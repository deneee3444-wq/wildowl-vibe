...
class SpamOK:
    ...
    @staticmethod
    def get_verify_link(username, max_attempts=45, log_callback=None):
        """15 dakikalık yapılandırmaya uygun olarak e-posta bekleme deneme sayısı artırıldı."""
        if log_callback:
            log_callback("SpamOK gelen kutusu taranıyor...", "registering", 25)
        for i in range(max_attempts):
            try:
                r = requests.get(
                    f"[api.spamok.com](https://api.spamok.com/v2/EmailBox/{username})",
                    timeout=15
                )
                if r.status_code == 200:
                    ...
                    if mails:
                        ...
                        msg_res = requests.get(
                            f"[api.spamok.com](https://api.spamok.com/v2/Email/{username}/{mail_id})",
                            timeout=15
                        )
...
class PopVidClient:
        ...
        self.session.headers.update(
            {
                ...
                "origin": "[popvid.ai](https://popvid.ai)",
                "referer": "[popvid.ai](https://popvid.ai/)",
            }
        )
    ...
    def signup_and_verify(self, log_callback=None):
        ...
        signup_url = f"[identitytoolkit.googleapis.com](https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key={API_KEY})"
        payload = {
            "requestType": "EMAIL_SIGNIN",
            "email": email,
            "clientType": "CLIENT_TYPE_WEB",
            "continueUrl": "[popvid.ai](https://popvid.ai/email-action?mode=signIn)",
            "canHandleCodeInApp": True,
        }
        res = self.session.post(signup_url, json=payload, timeout=30)
        ...
        if oob_code:
            ...
            login_url = f"[identitytoolkit.googleapis.com](https://identitytoolkit.googleapis.com/v1/accounts:signInWithEmailLink?key={API_KEY})"
            login_res = self.session.post(
                login_url, json={"email": email, "oobCode": code}, timeout=30
            )
            if login_res.status_code == 200:
                ...
                user_info_url = f"[popvid.ai](https://popvid.ai/api/v3/users/{self.user_id}?stableId={stable_id})"
                init_res = self.session.get(user_info_url, timeout=30)
                ...
                self.session.post(
                    "[popvid.ai](https://popvid.ai/api/v3/locale)", json={"locale": "en"}, timeout=15
                )
                time.sleep(1)
                return True

        self.session.get(verify_link, timeout=30)
    ...
    def upload_image(self, file_path, log_callback=None):
        ...
        url = "[popvid.ai](https://popvid.ai/api/imagecheck)"
        with open(file_path, "rb") as f:
            files = {"image": (os.path.basename(file_path), f, "image/jpeg")}
            data = {"useCase": ""}
            res = self.session.post(url, files=files, data=data, timeout=60)
    ...
    def generate_video(
        ...
        url = "[popvid.ai](https://popvid.ai/api/v3/create_a)"
    ...
    def extend_video(self, meme_id, prompt, character_type="human", log_callback=None):
        url = "[popvid.ai](https://popvid.ai/api/v3/extend)"
        prompt_b64 = base64.b64encode(prompt.encode("utf-8")).decode("utf-8")
    ...
    def poll_status(self, meme_id, req_type="text_prompt", timeout=900, log_callback=None):
        """
        15 Dakika (900 Saniye) Zaman Aşımı Yapılandırması:
        timeout parametresi varsayılan olarak 900 (15 dakika) yapıldı.
        """
        url = f"[popvid.ai](https://popvid.ai/api/v3/users/{self.user_id}/batch-status)"
        payload = {"requests": [{"memeId": meme_id, "type": req_type}]}

        if log_callback:
            log_callback("Video işleme durumu takip ediliyor (Polling - Max 15 dk)...", "generating", 88)
        ...
        while time.time() - start_time < timeout:
            ...
            time.sleep(3)  # Sunucuyu yormamak adına kontrol aralığı 3 saniyeye çıkarıldı

        if log_callback:
            log_callback("Video üretimi 15 dakikalık zaman aşımına (timeout) uğradı.", "error", 90)
        return False

    def get_preview_url(self, meme_id, log_callback=None):
        url = f"[popvid.ai](https://popvid.ai/api/v3/meme/{meme_id})"
...
def run_once(image_path: str, prompt: str, character_type: str = "human", log_callback=None, timeout: int = 900):
    """PopVid sıfırdan video üretimi fonksiyonu (15 Dakika Timeout Destekli)."""
    def _log(msg, status="info", pct=None):
    ...
    # 15 dakikalık timeout ile kontrol döngüsü
    success = client.poll_status(meme_id, req_type="text_prompt", timeout=timeout, log_callback=_log)
    if not success:
        raise RuntimeError("PopVid video işleme başarısız veya 15 dakikalık zaman aşımı aşıldı.")
...
def run_extend(meme_id: str, prompt: str, character_type: str = "human", user_id: str = None, id_token: str = None, log_callback=None, timeout: int = 900):
    """PopVid mevcut videoyu uzatma (Extend) fonksiyonu (15 Dakika Timeout Destekli)."""
    def _log(msg, status="info", pct=None):
    ...
    # 15 dakikalık timeout ile kontrol döngüsü
    success = client.poll_status(new_meme_id, req_type="extend", timeout=timeout, log_callback=_log)
    if not success:
        raise RuntimeError("PopVid video uzatma işlemi başarısız veya 15 dakikalık zaman aşımı aşıldı.")
