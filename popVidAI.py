...
# Video işleme için maksimum bekleme süresi (saniye) -> 15 dakika
POLL_TIMEOUT = 15 * 60
...
class PopVidClient:
    ...
    def signup_and_verify(self, log_callback=None):
        ...
        if oob_code:
            ...
            if login_res.status_code == 200:
                ...
                self.session.post(
                    "[popvid.ai](https://popvid.ai/api/v3/locale)", json={"locale": "en"}, timeout=10
                )
                time.sleep(1)
                return True
    ...
    def poll_status(self, meme_id, req_type="text_prompt", timeout=POLL_TIMEOUT, log_callback=None):
        url = f"[popvid.ai](https://popvid.ai/api/v3/users/{self.user_id}/batch-status)"
        payload = {"requests": [{"memeId": meme_id, "type": req_type}]}

        if log_callback:
            log_callback(f"Video işleme durumu takip ediliyor (Polling)... Maksimum bekleme: {timeout // 60} dk", "generating", 88)

        start_time = time.time()
        last_pct = 0
        consecutive_errors = 0
        max_consecutive_errors = 20  # Art arda 20 hata olursa pes et
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    if calc_pct > last_pct:
                        last_pct = calc_pct
                        if log_callback:
                            log_callback(f"Video işleniyor: %{progress} - {msg} (Kalan süre: {remaining // 60} dk {remaining % 60} sn)", "generating", calc_pct)
                    ...
                    print(f"[!] Polling HTTP hatası: {res.status_code} ({consecutive_errors}/{max_consecutive_errors})")
            except Exception as e:
                consecutive_errors += 1
                print(f"[!] Polling hatası: {e} ({consecutive_errors}/{max_consecutive_errors})")

            if consecutive_errors >= max_consecutive_errors:
                if log_callback:
                    log_callback("Polling art arda çok fazla hatayla karşılaştı, işlem iptal edildi.", "error", 90)
                return False

            time.sleep(2)

        if log_callback:
            log_callback(f"Video üretimi {timeout // 60} dakikalık zaman aşımına uğradı.", "error", 90)
        return False
...
def run_once(image_path: str, prompt: str, character_type: str = "human", log_callback=None):
    ...
    success = client.poll_status(meme_id, req_type="text_prompt", timeout=POLL_TIMEOUT, log_callback=_log)
    if not success:
        raise RuntimeError("PopVid video işleme başarısız veya 15 dakikalık zaman aşımı.")
...
def run_extend(meme_id: str, prompt: str, character_type: str = "human", user_id: str = None, id_token: str = None, log_callback=None):
    ...
    success = client.poll_status(new_meme_id, req_type="extend", timeout=POLL_TIMEOUT, log_callback=_log)
    if not success:
        raise RuntimeError("PopVid video uzatma işlemi başarısız veya 15 dakikalık zaman aşımı.")
